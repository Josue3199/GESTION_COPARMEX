...
// Navegadores integrados (WhatsApp, Instagram, Facebook, Messenger, TikTok,
// Line) donde Google bloquea el login por seguridad, aunque se vean igual
// que Safari o Chrome. Si detectamos uno, avisamos en vez de dejar que la
// persona choque con un error críptico de Firebase.
function esNavegadorIntegrado() {
  const ua = navigator.userAgent || ''
  return /FBAN|FBAV|Instagram|Line\/|MicroMessenger|WhatsApp|Twitter|TikTok|; ?wv\)/i.test(ua)
}

export default function Login() {
  const { loginConGoogle, logout, noAutorizado, user, errorLogin, setErrorLogin } = useAuth()
  const navegadorIntegrado = esNavegadorIntegrado()

  const handleGoogle = () => {
    setErrorLogin('')
    // signInWithRedirect saca de la app a la página de Google; el
    // resultado se recoge solo al volver (ver AuthContext).
    loginConGoogle()
  }
      ...
      <div className="w-full max-w-sm bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        {navegadorIntegrado && (
          <div className="bg-amber-50 border border-amber-200 text-amber-800 text-sm rounded-md p-3 mb-4">
            Parece que abriste este link desde WhatsApp, Instagram u otra app. Google no permite
            iniciar sesión ahí por seguridad. Toca <strong>⋮</strong> o el ícono de compartir y
            elige <strong>"Abrir en el navegador"</strong> (Safari o Chrome).
          </div>
        )}
        ...
        {user && noAutorizado ? (
          ...
          <>
            ...
            {errorLogin && <p className="text-sm text-red-600 mb-3 text-center">{errorLogin}</p>}

            <button
              onClick={handleGoogle}
              disabled={navegadorIntegrado}
              className="w-full flex items-center justify-center gap-2 border border-slate-300 hover:bg-slate-50 text-slate-700 font-medium py-2.5 rounded-md transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
